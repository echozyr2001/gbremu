...
mod tests {
  ...
  #[test]
  fn clock_ticks_according_to_period() {
    let period = Duration::from_millis(100); // 100 ms
    let mut clock = Clock::with_period(period);
    let start = Instant::now();
    // Wait for two ticks
    clock.next();
    clock.next();
    let elapsed = start.elapsed();
    // Check if elapsed time is approximately equal to two periods (allowing some margin for error)
    assert!(elapsed >= period * 2 && elapsed <= period * 2 + Duration::from_millis(50));
  }
}
