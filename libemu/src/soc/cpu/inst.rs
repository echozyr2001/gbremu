...
fn ld_mbc_a(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.bc(), cpu.regs.a);
}
...
fn ld_mu16_sp(cpu: &mut Cpu) {
  let word = cpu.read_u16();
  cpu.bus_mut().write(word, cpu.regs.sp as u8);
  cpu.bus_mut().write(word + 1, (cpu.regs.sp >> 8) as u8);
}
...
fn inc_c(cpu: &mut Cpu) {
  let c = cpu.regs.c;
  let value = cpu.regs.c.wrapping_add(1);
...
fn ld_mde_a(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.de(), cpu.regs.a);
}
...
fn rla(cpu: &mut Cpu) {
  ...
  cpu.set_carry(cpu.regs.a & 0x80 == 0x80);

  cpu.regs.a = cpu.regs.a << 1 | carry as u8;
...
fn jr_i8(cpu: &mut Cpu) {
  ...
  cpu.regs.pc = (cpu.regs.pc as i16).wrapping_add(byte as i16) as u16;
}
...
fn inc_e(cpu: &mut Cpu) {
  let e = cpu.regs.e;
  let value = cpu.regs.e.wrapping_add(1);
...
fn rra(cpu: &mut Cpu) {
  ...
  cpu.set_carry((cpu.regs.a & 0x1) == 0x1);

  cpu.regs.a = cpu.regs.a >> 1 | ((carry as u8) << 7);
...
fn inc_h(cpu: &mut Cpu) {
  let h = cpu.regs.h;
  let value = cpu.regs.h.wrapping_add(1);
...
fn inc_mhl(cpu: &mut Cpu) {
  ...
  cpu.bus_mut().write(cpu.hl(), value);
}
...
fn ld_mhl_b(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.b);
}

fn ld_mhl_c(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.c);
}

fn ld_mhl_d(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.d);
}

fn ld_mhl_e(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.e);
}

fn ld_mhl_h(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.h);
}

fn ld_mhl_l(cpu: &mut Cpu) {
  cpu.bus_mut().write(cpu.hl(), cpu.regs.l);
}
...
fn add_a_b(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.b);
}

fn add_a_c(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.c);
}

fn add_a_d(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.d);
}

fn add_a_e(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.e);
}

fn add_a_h(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.h);
}

fn add_a_l(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.l);
}

fn add_a_mhl(cpu: &mut Cpu) {
  let byte = cpu.bus().read(cpu.hl());
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, byte);
}

fn add_a_a(cpu: &mut Cpu) {
  cpu.regs.a = add_set_flags(cpu, cpu.regs.a, cpu.regs.a);
}

fn adc_a_b(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.b);
}

fn adc_a_c(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.c);
}

fn adc_a_d(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.d);
}

fn adc_a_e(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.e);
}

fn adc_a_h(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.h);
}

fn adc_a_l(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.l);
}

fn adc_a_mhl(cpu: &mut Cpu) {
  let byte = cpu.bus().read(cpu.hl());
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, byte);
}

fn adc_a_a(cpu: &mut Cpu) {
  cpu.regs.a = add_carry_set_flags(cpu, cpu.regs.a, cpu.regs.a);
}

fn sub_a_b(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.b);
}

fn sub_a_c(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.c);
}

fn sub_a_d(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.d);
}

fn sub_a_e(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.e);
}

fn sub_a_h(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.h);
}

fn sub_a_l(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.l);
}

fn sub_a_mhl(cpu: &mut Cpu) {
  let byte = cpu.bus().read(cpu.hl());
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, byte);
}

fn sub_a_a(cpu: &mut Cpu) {
  cpu.regs.a = sub_set_flags(cpu, cpu.regs.a, cpu.regs.a);
}

fn sbc_a_b(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.b);
}

fn sbc_a_c(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.c);
}

fn sbc_a_d(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.d);
}

fn sbc_a_e(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.e);
}

fn sbc_a_h(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.h);
}

fn sbc_a_l(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.l);
}

fn sbc_a_mhl(cpu: &mut Cpu) {
  let byte = cpu.bus().read(cpu.hl());
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, byte);
}

fn sbc_a_a(cpu: &mut Cpu) {
  cpu.regs.a = sub_carry_set_flags(cpu, cpu.regs.a, cpu.regs.a);
}
...
fn cp_a_b(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.b);
}

fn cp_a_c(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.c);
}

fn cp_a_d(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.d);
}

fn cp_a_e(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.e);
}

fn cp_a_h(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.h);
}

fn cp_a_l(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.l);
}

fn cp_a_mhl(cpu: &mut Cpu) {
  let byte = cpu.bus().read(cpu.hl());
  sub_set_flags(cpu, cpu.regs.a, byte);
}

fn cp_a_a(cpu: &mut Cpu) {
  sub_set_flags(cpu, cpu.regs.a, cpu.regs.a);
}
...
fn ld_mff00u8_a(cpu: &mut Cpu) {
  let byte = cpu.read_u8();
  cpu.bus_mut().write(0xff00 + byte as u16, cpu.regs.a);
}
...
fn ld_mff00c_a(cpu: &mut Cpu) {
  cpu.bus_mut().write(0xff00 + cpu.regs.c as u16, cpu.regs.a);
}
...
fn ld_mu16_a(cpu: &mut Cpu) {
  let word = cpu.read_u16();
  cpu.bus_mut().write(word, cpu.regs.a);
}
...
fn ld_a_mff00u8(cpu: &mut Cpu) {
  let byte = cpu.read_u8();
  let value = cpu.bus().read(0xff00 + byte as u16);
  cpu.regs.a = value;
}
...
fn ld_a_mff00c(cpu: &mut Cpu) {
  let value = cpu.bus().read(0xff00 + cpu.regs.c as u16);
  cpu.regs.a = value;
}
...
fn rlc_mhl(cpu: &mut Cpu) {
  let hl = cpu.hl();
  let byte = cpu.bus().read(hl);
  let result = rlc(cpu, byte);
  cpu.bus_mut().write(hl, result);
}
...
fn bit_a(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.a, bit));
  cpu.set_half_carry(true);
}

fn bit_b(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.b, bit));
  cpu.set_half_carry(true);
}

fn bit_c(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.c, bit));
  cpu.set_half_carry(true);
}

fn bit_d(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.d, bit));
  cpu.set_half_carry(true);
}

fn bit_e(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.e, bit));
  cpu.set_half_carry(true);
}

fn bit_h(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.h, bit));
  cpu.set_half_carry(true);
}

fn bit_l(cpu: &mut Cpu, bit: u8) {
  cpu.set_sub(false);
  cpu.set_zero(bit_zero(cpu.regs.l, bit));
  cpu.set_half_carry(true);
}
